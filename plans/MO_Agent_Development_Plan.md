...
**⚠️ CALORIC RAMP-UP PROTOCOL (do NOT jump to full target from day 1):**

| Phase | Weeks | Daily Target | Surplus vs Estimated Current (~1,800 kcal) | How |
|---|---|---|---|---|
| Ramp 1 | Week 1 | ~2,100 kcal | +300 | Add 1 daily shake (~300 kcal) or calorie-dense snack |
| Ramp 2 | Week 2 | ~2,300 kcal | +500 | Increase portion sizes, add olive oil/cheese/butter toppings |
| Full protocol | Week 3+ | ~2,450-2,550 kcal | +650-750 vs current / +400-500 vs training TDEE | Full 5-meal template |

Rationale: (1) GI adaptation — gut needs time to adjust to increased volume. (2) NEAT dampening — gradual surplus reduces unconscious activity compensation. (3) Psychological — prevents food aversion from sudden overconsumption. (4) Clinical guidelines (Friedli et al. 2020): recommend +200-300 kcal every 3-5 days for at-risk populations.

**⚠️ CREATINE TIMING AND SCALE INTERPRETATION:**

Creatine causes 1-2 kg intracellular water weight gain in the first 1-3 weeks. To prevent false adjustment triggers:
- **Option A (preferred)**: Start creatine 2-3 weeks BEFORE the caloric surplus begins. Water weight stabilizes before surplus tracking starts.
- **Option B**: Start creatine with surplus but designate **Weeks 1-4 as "adaptation period"** — collect data, NO adjustment triggers applied. First adjustment decision at Week 5.
- Regardless of option: **never trigger calorie reduction based on weight gain in weeks 1-4.**

**Macronutrient targets (calculated, not strategic):**

| Macro | Target | Daily Amount at 55 kg | Recalculation | Evidence |
|---|---|---|---|---|
| Protein | 1.6-2.0 g/kg/day | 88-110 g (352-440 kcal) | Recalc monthly using CURRENT weight (at 60 kg → 96-120g; at 65 kg → 104-130g) | Morton et al. 2018: breakpoint at 1.62 g/kg (49 studies, 1,863 participants) |
| Fat | ≥25% of calories | ~69 g (621 kcal) | Scales with total calories | Mumford et al. 2016: +4% testosterone, -58% anovulation risk. Below 20% → menstrual disruption |
| Carbohydrates | Remainder | ~220-275 g (880-1100 kcal) | Scales with total calories | Glycogen adequacy supports mTOR; Henselmans 2022: CHO unlikely to limit RT performance when fed |
| **Fiber** | **≥20g/day minimum** | **20-25g** | **Constant** | **GI health requirement. Low fiber + calorie-dense diet → constipation → bloating → less eating → failed surplus.** |

**Hydration target**: ~2.0-2.5L water/day. Increases with creatine supplementation (osmotically active). Practical: 1 glass on waking, 1 after each meal, 1 during training, 1 before bed.

**Progress metrics:**
- Target weight gain rate: 0.25-0.5 kg/week (measured AFTER adaptation period)
...
**⚠️ MENSTRUAL CYCLE-AWARE TRACKING:**
- Women retain 0.5-2 kg water during luteal phase
- If cycle is tracked: compare weight from same cycle phase (follicular Week 1 vs previous follicular Week 1)
- If cycle is not tracked or irregular: use **4-week rolling average** instead of 2-week triggers
- Menstrual restoration (if previously absent/irregular) is a POSITIVE progress marker — log and celebrate it

**Adjustment trigger rules (SCIENTIST detects, other agents execute):**
**NOTE: No adjustments during Weeks 1-4 (adaptation period). First adjustment decision at Week 5.**

| Trigger | Detection | Action Owner |
|---|---|---|
| Gain < 0.25 kg/week × 2 consecutive weeks (post-adaptation) | SCIENTIST (weekly average, cycle-adjusted) | SCIENTIST recalculates (+200 kcal) → DIETITIAN adjusts plan → CHEF adjusts recipes |
| Gain > 0.75 kg/week (post-adaptation, week 5+) | SCIENTIST (weekly average) | SCIENTIST recalculates (-150 kcal) → DIETITIAN adjusts plan |
...
| Protein target recalculation | SCIENTIST (monthly with weigh-in) | SCIENTIST updates → NUTRITIONIST/DIETITIAN adjust |

**Body composition baseline (recommended):**
- Waist-to-hip ratio at intake
- If available: DEXA or bioimpedance for body fat baseline
...
**⚠️ Tiered Meal Plans (aligned with SCIENTIST caloric ramp-up):**

| Tier | When | Daily Target | Meals | Difference from Previous |
|---|---|---|---|---|
| Tier 0 | Week 1 | ~2,100 kcal | Keep current eating habits + add 1 daily shake (~300 kcal) | Minimal disruption — only ADD, don't restructure |
| Tier 1 | Week 2 | ~2,300 kcal | Current habits + shake + calorie-dense additions (olive oil, cheese, butter, nuts on existing meals) | +200 kcal through stealth calories on existing food |
| Tier 2 | Week 3+ | ~2,450-2,650 kcal | Full 5-meal template above | Full protocol activated |

**⚠️ "Minimum Viable Day" Emergency Protocol:**

For days when appetite is zero (illness, stress, exhaustion, emotional difficulty):
...
- Maximum 2 minimum-viable days per week. If >2, flag to SCIENTIST as compliance issue.
- Minimum viable day prevents muscle loss and maintains hormonal status.
- Always prioritize liquid calories when solid food is difficult.

**⚠️ "Solo Week" Protocol (when partner is unavailable):**
- All meals must be achievable by user alone with basic cooking skills
- Rely on: rotisserie chicken, pre-cooked rice, canned tuna/salmon, eggs, frozen meals (macro-checked), extra shakes
- CHEF must provide 5-6 "survival recipes" — max 5 steps, max 15 min prep

**Food tracking method:**
- Weeks 1-4: Food scale + simple app (MyFitnessPal) for learning portion sizes
- Weeks 5+: Transition to portion estimation (palm = protein, fist = carbs, thumb = fats)
- Re-engage full tracking only when SCIENTIST detects stall and needs to verify intake
...
**⚠️ Weight increment correction for untrained women:**
- Upper body: **+1 kg per dumbbell** or **+1.25 kg per side barbell** (NOT +2.5 kg total — that's too aggressive at starting weights of 5-15 kg)
...
**⚠️ Warm-Up Protocol (MANDATORY — included in every session):**
...
## ⚠️ Psychological Preparation for Intentional Fat Gain (Cross-Cutting)

**This subject will gain 5-7 kg of fat intentionally.** Even with favorable estrogen-directed distribution, she WILL notice her belly, face, and thighs getting bigger BEFORE she sees visible muscle definition. This is the most psychologically vulnerable phase of the protocol.
...
**Warning signs that cross from normal discomfort to red flag:**
- Normal: feeling a bit soft, occasional frustration, wanting to see results faster
- RED FLAG: panic attacks about eating, purging behaviors, compulsive cardio addition, calorie restriction despite being underweight, body checking >5x/day, avoiding social situations due to body image → invoke PHYSICIAN → referral to eating disorder specialist

**The surplus is literally healing her body.** Frame it as medical restoration, not cosmetic excess.

## ⚠️ Recommended Baseline Blood Work (PHYSICIAN Advisory)

Not mandatory to begin the protocol, but strongly recommended for an underweight woman starting a training program. Results may change supplement dosing and catch subclinical issues early.

| Panel | Why | What It Catches |
|---|---|---|
| Complete Blood Count (CBC) | Screen for anemia (common in underweight women) | Low hemoglobin/hematocrit → poor oxygen delivery → reduced exercise tolerance |
| Ferritin | Iron stores (more sensitive than CBC for early depletion) | <30 ng/mL = functional deficiency. Supplement iron if low. |
| 25-OH Vitamin D | Baseline for D3 dosing | <20 ng/mL = deficient (may need loading dose 50,000 IU/wk × 8 wks). 20-40 = insufficient. >40 = adequate. |
| TSH + free T3 + free T4 | Thyroid function | Subclinical hypothyroidism (elevated TSH) or sick euthyroid (low T3, normal TSH — common in energy insufficiency) |
| Basic Metabolic Panel | Electrolytes, kidney, glucose | Baseline safety. Rules out metabolic issues before supplement loading. |
| *Optional*: Estradiol, progesterone, SHBG | Reproductive hormone status | Only if menstrual irregularity reported. Confirms HPG axis suppression level. |

**Where to go**: GP or endocrinologist. Frame to patient: "This is like checking your car's fluids before a road trip. Everything might be fine, but knowing your baseline lets us make smarter decisions."
